...
:: Accepted and ignored. The launcher passes the same flags to both
:: scripts; only uninstall.cmd acts on this one, and rejecting it here
:: would fail an install for a flag that simply does not apply.
...
:: ============================================
:: UTC ISO-8601. Via PowerShell, which this script already requires for game
:: detection - a plain %DATE% is whatever the user's regional settings say and
:: is not parseable, and WMIC is no longer present on current Windows 11.
:stamp_installed_at
set "INSTALLED_AT="
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "[DateTime]::UtcNow.ToString('yyyy-MM-ddTHH:mm:ssZ')" 2^>nul`) do (
    set "INSTALLED_AT=%%I"
)
if not defined INSTALLED_AT set "INSTALLED_AT=1970-01-01T00:00:00Z"
exit /b 0
...
> "%GAME_PATH%\%STATE_FILE%" (
    ...
    echo     "version": "%ASI_LOADER_VERSION%"
    echo   },
    echo   "mod": {
    echo     "id": "%GAME_ID%",
